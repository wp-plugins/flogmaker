...
if(!$resultPage){
	...
	foreach($resultPage as $e){	
		...
		$reqCategorie="SELECT term_id FROM {$table_prefix}terms WHERE term_id IN (SELECT term_id FROM {$table_prefix}term_taxonomy WHERE taxonomy='category') AND term_id IN (SELECT term_taxonomy_id FROM wp_term_relationships WHERE object_id=".$e->ID.")";
		...
		$somecontent = $somecontent."<loc><![CDATA[".$myURL."/index.php?idCat=".$idmacat."&idArticle=".$e->ID."&page=".urlencode($e->post_title)."]]></loc>";
