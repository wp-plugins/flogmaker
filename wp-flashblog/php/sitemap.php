...
if(!$resultPage){
	...
	foreach($resultPage as $e){	
		...
		$reqCategorie="SELECT term_id FROM wp_terms WHERE term_id IN (SELECT term_taxonomy_id FROM wp_term_relationships WHERE object_id =".$e->ID." AND term_taxonomy_id IN(SELECT term_id FROM wp_term_taxonomy WHERE taxonomy='category'))";
